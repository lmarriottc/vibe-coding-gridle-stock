...
async function submitMovement({ type, model, size, quantityInput, priceInput, submitButton, message }) {
  message.textContent = "";
  ...
  try {
    await recordMovement({ type, model: model.value, size: size.value, quantity, unitPrice });
    if (getSheetUrl()) await loadData(); else { populateForms(); render(); }
    quantityInput.value = 1; priceInput.value = "";
...
function readMovementItems() {
  return [...document.querySelectorAll("#movementItems .movement-item")].map((item) => ({
    type: el.type.value,
...
export function initializeForms() {
  ...
  $("#newSaleButton").addEventListener("click", () => { populateSaleModels(); el.saleMessage.textContent = ""; el.saleDialog.showModal(); el.saleModel.focus(); });
  $("#closeSaleButton").addEventListener("click", () => el.saleDialog.close());
  el.saleForm.addEventListener("submit", async (event) => {
    event.preventDefault();
    const saved = await submitMovement({ type: "sale", model: el.saleModel, size: el.saleSize, quantityInput: el.saleQty, priceInput: el.salePrice, submitButton: el.saleSubmit, message: el.saleMessage });
